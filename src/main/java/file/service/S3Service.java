...
public class S3Service {
	...
	private final AmazonS3 amazonS3;  // bucket과 file 관련된 기능 메서드 존재하는 라이브러리
	...
	public void uploadS3File(MultipartFile file) throws Exception {
		...
		// s3에 물리적으로 저장
		if(fileNo != null) {
			// 임시 파일 저장, 왜 임시로 저장해야지만 s3에 저장 가능함?? -> 로컬에 파일이 임시적으로 저장되어야지만 다른 서버로 파일을 전달할 수 있다.
			File uploadFile = new File(attachmentFile.getFilePath() + "//" + attachmentFileName);
			file.transferTo(uploadFile);
			
			// s3 파일 전송, bucket 이름/파일 이름 이용
			// bucket : 버킷
			// key : 객체의 저장 경로 + 객체의 이름
			// file : 물리적인 리소스
			String key = DIR_NAME + "/" + uploadFile.getName();
			amazonS3.putObject(bucketName, key, uploadFile); // 업로드하는 메서드
			
			// S3에 저장이 되면 로컬에 있는 임시 파일 삭제
